...
## These git commands will fail in the submodule since its .git dir is actually in the parent
...
cd ..
tar czf rippled-${RIPPLED_DPKG_VERSION}.tar.gz rippled
...
mkdir rippled-${RIPPLED_DPKG_VERSION} && tar xvf rippled-${RIPPLED_DPKG_VERSION}.tar.gz -C rippled-${RIPPLED_DPKG_VERSION} --strip-components=1 # why does this even happen? Just to make sure it's all correct?
